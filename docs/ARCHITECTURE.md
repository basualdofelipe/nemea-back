...
A demo login path exists (`POST /api/auth/demo`) that is active only when `DEMO_LOGIN_ENABLED=true`. It is hard-pinned to `demo@nemea.app` — the endpoint rejects any other email even if `DEMO_LOGIN_ENABLED` is true, so it cannot be used to mint tokens for real accounts.
